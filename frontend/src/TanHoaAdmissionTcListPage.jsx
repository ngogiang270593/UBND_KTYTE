...
const columns = [
  ["stt", "STT"], ["hoTen", "Họ Tên BN"], ["soTheBhyt", "Số thẻ BHYT"], ["namSinh", "Năm Sinh"],
  ["gioiTinh", "Giới Tính"], ["ngheNghiep", "Nghề Nghiệp"], ["diaChi", "Địa Chỉ"], ["chanDoanVaoVien", "Chẩn đoán/Vào viện"],
];
const normalize = (value) => String(value ?? "").normalize("NFD")
  .replace(/[\u0300-\u036f]/g, "").replace(/đ/gi, "d").toLowerCase().trim();
const pageSize = 50;

export default function TanHoaAdmissionTcListPage() {
  ...
  useEffect(() => {
    let active = true;
    api.get("/TanHoaAdmissionTc").then(({ data }) => {
      if (active) setRows(data || []);
    }).catch(() => {
      if (active) setMessage("Không tải được danh sách nhập viện TC Tân Hòa.");
    }).finally(() => {
  ...
  const reload = async () => {
    ...
    setMessage("");
    try {
      const { data } = await api.get("/TanHoaAdmissionTc");
      setRows(data || []);
    } catch {
  ...
  const exportExcel = () => {
    try {
      ...
      sheet["!cols"] = [{ wch: 8 }, { wch: 30 }, { wch: 22 }, { wch: 12 }, { wch: 12 }, { wch: 25 }, { wch: 45 }, { wch: 45 }];
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, sheet, "Nhap vien TC");
      XLSX.writeFile(workbook, `Nhap_vien_TC_Tan_Hoa_${new Date().toISOString().slice(0, 10)}.xlsx`);
  ...
  const deleteData = async (row) => {
    ...
    try {
      const accepted = await confirm({
        title: row ? "Xóa dòng dữ liệu?" : "Xóa toàn bộ dữ liệu import nhập viện TC Tân Hòa?",
        message: row
          ? `Xóa dòng của “${row.hoTen || row.soTheBhyt || "chưa có họ tên"}”? Không thể hoàn tác.`
          : `Xóa toàn bộ ${rows.length} dòng đã import nhập viện TC của Tân Hòa, bao gồm các dòng ngoài kết quả tìm kiếm. Không thể hoàn tác.`,
        confirmText: row ? "Xóa" : "Xóa toàn bộ",
      });
      if (!accepted) return;
      const { data } = await api.delete(row ? `/TanHoaAdmissionTc/${row.id}` : "/TanHoaAdmissionTc/all");
      setRows((previous) => row ? previous.filter((item) => item.id !== row.id) : []);
      setMessage(`Đã xóa ${data.count} dòng dữ liệu import nhập viện TC Tân Hòa.`);
    } catch (error) {
  ...
  return <div className="container-fluid px-0">
    <div className="card shadow-sm mb-4"><div className="card-body">
      <h5 className="fw-bold text-primary">Danh sách nhập viện TC Tân Hòa</h5>
      <p className="text-muted">Dữ liệu đã lưu từ menu Import dữ liệu nhập viện TC của module Tân Hòa.</p>
      <div className="d-flex gap-2 flex-wrap">
        <input aria-label="Tìm kiếm danh sách nhập viện TC" className="form-control flex-grow-1" style={{ maxWidth: 600 }}
          placeholder="Tìm theo STT, họ tên, BHYT, năm sinh, giới tính, nghề nghiệp, địa chỉ, chẩn đoán..."
    ...
    <div className="card shadow-sm">
      ...
      <div className="table-responsive"><table className="table table-bordered table-hover table-sm align-middle mb-0">
        ...
        {!filtered.length && <tr><td colSpan={9} className="text-center text-muted py-4">{busy ? "Đang tải..." : "Không có dữ liệu"}</td></tr>}</tbody>
      </table></div>
