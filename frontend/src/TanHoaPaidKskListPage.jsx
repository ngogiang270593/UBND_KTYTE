...
export default function TanHoaPaidKskListPage() {
  ...
  useEffect(() => {
    let active = true;
    api.get("/TanHoaPaidKsk").then(({ data }) => {
      if (active) setRows(data || []);
    }).catch(() => {
      if (active) setMessage("Không tải được danh sách KSK (đóng phí) Tân Hòa.");
    }).finally(() => {
  ...
  const reload = async () => {
    ...
    setMessage("");
    try {
      const { data } = await api.get("/TanHoaPaidKsk");
      setRows(data || []);
    } catch {
  ...
  const exportExcel = () => {
    try {
      ...
      sheet["!cols"] = [{ wch: 8 }, { wch: 30 }, { wch: 12 }, { wch: 12 }, { wch: 20 }, { wch: 45 }];
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, sheet, "KSK dong phi");
      XLSX.writeFile(workbook, `KSK_dong_phi_Tan_Hoa_${new Date().toISOString().slice(0, 10)}.xlsx`);
  ...
  const deleteData = async (row) => {
    ...
    try {
      const accepted = await confirm({
        title: row ? "Xóa dòng dữ liệu?" : "Xóa toàn bộ dữ liệu import KSK (đóng phí) Tân Hòa?",
        message: row
          ? `Xóa dòng của “${row.hoTen || row.cccd || "chưa có họ tên"}”? Không thể hoàn tác.`
          : `Xóa toàn bộ ${rows.length} dòng đã import KSK (đóng phí) của Tân Hòa, bao gồm các dòng ngoài kết quả tìm kiếm. Không thể hoàn tác.`,
        confirmText: row ? "Xóa" : "Xóa toàn bộ",
      });
      if (!accepted) return;
      const { data } = await api.delete(row ? `/TanHoaPaidKsk/${row.id}` : "/TanHoaPaidKsk/all");
      setRows((previous) => row ? previous.filter((item) => item.id !== row.id) : []);
      setMessage(`Đã xóa ${data.count} dòng dữ liệu import KSK (đóng phí) Tân Hòa.`);
    } catch (error) {
  ...
  return <div className="container-fluid px-0">
    <div className="card shadow-sm mb-4"><div className="card-body">
      <h5 className="fw-bold text-primary">Danh sách KSK (đóng phí) Tân Hòa</h5>
      <p className="text-muted">Dữ liệu đã lưu từ menu Import dữ liệu KSK (đóng phí) của module Tân Hòa.</p>
      <div className="d-flex gap-2 flex-wrap">
        <input aria-label="Tìm kiếm danh sách KSK (đóng phí)" className="form-control flex-grow-1" style={{ maxWidth: 600 }}
          placeholder="Tìm theo STT, họ tên, năm sinh, giới tính, CCCD, địa chỉ..."
