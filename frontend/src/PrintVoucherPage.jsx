...
export default function PrintVoucherPage() {
  ...
  const loadData = async (filters = emptyForm) => { setLoading(true); setMessage(""); try { const response = await api.get("/PrintVoucher/customers", { params: buildParams(filters) }); setCustomers(Array.isArray(response.data) ? response.data : []); setActiveTab("all"); setPage(1); } catch { setCustomers([]); setMessage("Không tải được dữ liệu in ấn."); } finally { setLoading(false); } };
  useEffect(() => { api.get("/CatalogItems", { params: { category: "hamlet" } }).then((response) => setHamlets(Array.isArray(response.data) ? response.data : [])).catch(() => setMessage("Không tải được danh mục ấp.")); loadData(); }, []);
  ...
  const tabs = [{ key: "all", label: "Tổng" }, ...hamlets.map((item) => ({ key: `hamlet-${item.id}`, label: item.name })), { key: "unknown", label: "Chưa có thông tin" }];
