...
export default function TanHoaInpatientListPage() {
  ...
  useEffect(() => {
    let active = true;
    api.get("/TanHoa").then(({ data }) => {
      if (active) setRows(data || []);
    }).catch(() => {
      if (active) setMessage("Không tải được danh sách nội trú Tân Hòa.");
    }).finally(() => {
  ...
  const reload = async () => {
    ...
    setMessage("");
    try {
      const { data } = await api.get("/TanHoa");
      setRows(data || []);
    } catch {
  ...
  const deleteData = async (row) => {
    ...
    try {
      const accepted = await confirm({
        title: row ? "Xóa dòng dữ liệu?" : "Xóa toàn bộ dữ liệu import Tân Hòa?",
        message: row
          ? `Xóa dòng của “${row.hoTen || row.cccd || "chưa có họ tên"}”? Không thể hoàn tác.`
          : `Xóa toàn bộ ${rows.length} dòng đã import của Tân Hòa, bao gồm các dòng ngoài kết quả tìm kiếm. Không thể hoàn tác.`,
        confirmText: row ? "Xóa" : "Xóa toàn bộ",
      });
      if (!accepted) return;
      const { data } = await api.delete(row ? `/TanHoa/${row.id}` : "/TanHoa/all");
      setRows((previous) => row ? previous.filter((item) => item.id !== row.id) : []);
      setMessage(`Đã xóa ${data.count} dòng dữ liệu import Tân Hòa.`);
    } catch (error) {
  ...
  return <div className="container-fluid px-0">
    <div className="card shadow-sm mb-4"><div className="card-body">
      <h5 className="fw-bold text-primary">Danh sách nội trú Tân Hòa</h5>
      <p className="text-muted">Dữ liệu đã lưu từ menu Import dữ liệu của module Tân Hòa.</p>
      <div className="d-flex gap-2 flex-wrap">
        <input aria-label="Tìm kiếm danh sách nội trú" className="form-control flex-grow-1" style={{ maxWidth: 600 }}
