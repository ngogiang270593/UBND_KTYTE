...
export default function TanHoaAdmissionTcImportPage() {
  ...
  const save = async () => {
    ...
    setMessage("");
    try {
      const response = await api.post("/TanHoaAdmissionTc/import", rows);
      setMessage(`Đã import thành công ${response.data.savedCount} dòng nhập viện TC Tân Hòa.`);
      setRows([]);
  ...
  return <div className="container-fluid px-0">
    <div className="card shadow-sm mb-4"><div className="card-body">
      <h5 className="fw-bold text-primary">Import nhập viện TC Tân Hòa</h5>
      <p className="text-muted">Dòng 1 là tiêu đề. Dữ liệu từ dòng 2, cột A đến H theo thứ tự: STT, Họ Tên BN, Số thẻ BHYT, Năm Sinh, Giới Tính, Nghề Nghiệp, Địa Chỉ, Chẩn đoán/Vào viện.</p>
      ...
      <div className="row g-3 align-items-end">
        <div className="col-md-9"><label htmlFor="tan-hoa-admission-tc-file" className="form-label">File Excel</label><input id="tan-hoa-admission-tc-file" ref={inputRef} type="file" accept=".xlsx,.xls" className="form-control" disabled={busy} onChange={readFile} /></div>
    ...
    <div className="mt-4"><TanHoaAdmissionTcListPage key={savedVersion} /></div>
  </div>;
}